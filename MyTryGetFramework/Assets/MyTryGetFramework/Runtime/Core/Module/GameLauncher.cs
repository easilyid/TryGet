...
namespace TryGet
{
    ...
    /// 框架启动工具集（V0.7 起新增）。
    ///
    /// 用途：标准化 <see cref="ModuleSystem"/> 创建 + 注册 Core 基础服务（<see cref="ILogger"/> +
    /// <see cref="IClock"/> + <see cref="ITGTaskScheduler"/>）的样板。业务调用 <see cref="CreateHost"/>
    /// 拿到预配置 host，再注册自己的 Module 和 <see cref="IModuleSystem.Initialize"/>。
    ///
    ...
    /// 形态本质不同，强行抽象 interface 增加心智成本不带来灵活性。GameLauncher 只做"创建并预填基础 Module"。
    ///
    ...
    {
        /// <summary>
        /// 创建 ModuleSystem 并注册 Core 基础三件套（<see cref="ILogger"/> + <see cref="IClock"/> +
        /// <see cref="ITGTaskScheduler"/>）。
        /// V0.9.5 起：调用 <see cref="ModuleRegistry.ApplyAll"/> 应用所有 Source Generator
        /// 自动生成的 Module 注册。业务调用后可继续手动 <see cref="IModuleSystem.Register{T}"/> 自己的
        /// Module，最后调 <see cref="IModuleSystem.Initialize"/>。
        /// </summary>
        /// <param name="options">可选配置，允许业务注入自定义 Logger/Clock/Scheduler。</param>
        public static IModuleSystem CreateHost(GameLauncherOptions options = null)
        {
            options ??= GameLauncherOptions.Default;
            ...
            // 阶段累计的 [Module] 自动注册委托
            ModuleRegistry.ApplyAll(host);

            // V0.9.5：应用 [EventHandler] 自动 Subscribe 到 host.EventModule
            EventHandlerRegistry.ApplyAll(host.EventModule);
