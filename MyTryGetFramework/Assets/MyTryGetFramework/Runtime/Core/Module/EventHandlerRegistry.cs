...
{
    /// <summary>
    /// V0.9.5 起：Source Generator 自动 Subscribe EventBus handler 的运行时收集器。
    ///
    /// 工作流：
    ...
    /// 2. <see cref="Bootstrap.CreateHost"/> 在 host 创建后调 <see cref="ApplyAll"/> 把
    ///    所有 handler Subscribe 到 host.EventBus
    /// 3. 业务可继续手动 <see cref="IEventBus.Subscribe{T}"/> 自己的 handler
    /// </summary>
    public static class EventHandlerRegistry
    {
        private static readonly List<Action<IEventBus>> _registrations = new List<Action<IEventBus>>();
        ...
        /// <summary>
        /// 对指定 EventBus 应用所有已注册的委托。<see cref="Bootstrap.CreateHost"/> 内部调用。
        /// </summary>
        public static void ApplyAll(IEventBus bus)
        {
