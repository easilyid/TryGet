...
namespace TryGet
{
    ...
    /// 2. <see cref="Bootstrap.CreateHost"/> 在 host 基础服务注册完成后调 <see cref="ApplyAll"/>
    ///    把累计的所有委托应用到新 host 上
    /// 3. 业务可继续手动 <see cref="IModuleHost.Register{T}"/> 自己的 Module
    /// 4. 最终调 <see cref="IModuleHost.Initialize"/>
    ///
    /// **线程安全**：仅在主线程调用（与 ModuleHost 一致）。<c>[ModuleInitializer]</c> 在
    ...
    /// </summary>
    public static class AssemblyManifestRegistry
    {
        private static readonly List<Action<IModuleHost>> _registrations = new List<Action<IModuleHost>>();

        /// <summary>
        /// 注册一个 Module-注册委托。生成代码（<c>__AssemblyManifest_&lt;asm&gt;</c>）调此方法。
        /// 业务一般不应直接调；业务使用 <see cref="ModuleAttribute"/> 让 Generator 自动生成。
        /// </summary>
        public static void Register(Action<IModuleHost> registration)
        {
        ...
        /// <summary>
        /// 对指定 host 应用所有已注册的委托。<see cref="Bootstrap.CreateHost"/> 内部调用。
        /// </summary>
        public static void ApplyAll(IModuleHost host)
        {
