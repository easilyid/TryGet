...
namespace TryGet
{
    ...
    /// 非线程安全：所有方法须在主线程调用。
    /// </summary>
    public sealed class ModuleSystem : IModuleSystem
    {
        ...
        private readonly IEventModule _eventBus = new EventModule();
        ...
        public IEventModule EventModule => _eventBus;
        ...
        /// 是丢弃 ModuleSystem 实例并重新构造。重复 Initialize 已初始化的 host 抛 <see cref="InvalidOperationException"/>。
        /// </summary>
        public void Initialize()
        {
            if (_initialized)
                throw new InvalidOperationException("ModuleSystem already initialized.");
