...
namespace TryGet
{
    ...
    /// - DependsOn 显式声明前置依赖，ModuleHost 据此拓扑排序
    ...
    public interface IModule
    {
        ...
        /// <summary>
        /// 显式依赖的其他 Module 接口类型。ModuleHost 据此拓扑排序，并检测循环依赖。
        /// 返回空集合表示无依赖。
        /// </summary>
        IReadOnlyList<Type> DependsOn { get; }

        /// <summary>
        /// 由 ModuleHost 在所有依赖都已 OnInit 后调用。允许通过 host.Get&lt;T&gt;() 拉依赖。
        /// </summary>
        void OnInit(IModuleHost host);

        /// <summary>
        /// 由 ModuleHost 在关闭流程中按 OnInit 的逆序调用。Module 应在此清理资源。
        /// </summary>
        void Shutdown();
    }
}
