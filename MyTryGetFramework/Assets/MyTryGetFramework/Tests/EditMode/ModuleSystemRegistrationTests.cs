...
{
    /// <summary>
    /// ModuleSystem 注册与查询测试。
    /// </summary>
    [TestFixture]
    public class ModuleSystemRegistrationTests
    {
        ...
        public void Register_SameInterfaceTwice_Throws()
        {
            ...
            Assert.Throws<ModuleAlreadyRegisteredException>(() =>
                host.Register<IFooModule>(new FooModule()));
        }
        ...
        public void Get_UnregisteredInterface_Throws()
        {
            ...
            Assert.Throws<ModuleNotRegisteredException>(() =>
                host.Get<IFooModule>());
        }
        ...
        [Test]
        public void EventModule_NonNullAndUsable()
        {
            // EventModule 在 ModuleSystem 构造时即可用，不需要 Initialize。
            var host = new ModuleSystem();
            int received = 0;
            host.EventModule.Subscribe<SampleEvent>(evt => received = evt.Value);

            host.EventModule.Publish(new SampleEvent { Value = 42 });
